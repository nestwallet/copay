...
angular.module('nestApp.controllers').controller('txpDetailsController', function($scope, $rootScope, $timeout, $interval, $log, ongoingProcess, platformInfo, $ionicScrollDelegate, txFormatService, bwcError, gettextCatalog, lodash, walletService, popupService, $ionicHistory, feeService) {
  ...
  $scope.init = function() {
    ...
    $scope.nesters = $scope.wallet.status.wallet.nesters;
    $scope.nesterId = $scope.wallet.credentials.nesterId;
  ...
  var updateTxInfo = function(eventName) {
    $scope.wallet.getTx($scope.tx.id, function(err, tx) {
      ...
      var action = lodash.find(tx.actions, {
        nesterId: $scope.wallet.credentials.nesterId
      });
  ...
  $scope.updateNesterList = function() {
    lodash.map($scope.nesters, function(cp) {
      lodash.each($scope.tx.actions, function(ac) {
        if (cp.id == ac.nesterId) {
          cp.action = ac.type;
        }
      });
    });
