...
angular.module('nestApp.controllers').controller('walletDetailsController', function($scope, $rootScope, $interval, $timeout, $filter, $log, $ionicModal, $ionicPopover, $state, $stateParams, $ionicHistory, profileService, lodash, configService, platformInfo, walletService, txpModalService, externalLinkService, popupService, addressbookService, storageService, $ionicScrollDelegate, $window, bwcError, gettextCatalog, timeService, feeService, appConfigService) {
  ...
  $scope.$on("$ionicView.afterLeave", function(event, data) {
    ...
    if ($window.StatusBar) {
      var statusBarColor = appConfigService.name == 'nest' ? '#192c3a' : '#1e3186';
  ...
  function setAndroidStatusBarColor() {
    ...
    var walletColor;
    if (!$scope.wallet.color) walletColor = appConfigService.name == 'nest' ? '#019477' : '#4a90e2';
